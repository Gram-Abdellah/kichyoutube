    ...
    try:
        ...
        try:
            ...
            if not segments:
                ...
                return "No segments found in m3u8 playlist. The URL might be invalid or expired."
        ...
        if start_sec > segments[-1]['end']:
            error = f"Start time {start_time} ({start_sec}s) is beyond the stream end ({segments[-1]['end']:.1f}s)"
            print(f"❌ {error}")
            return error

        if end_sec > segments[-1]['end']:
            print(f"⚠️ End time exceeds stream, capping to {segments[-1]['end']:.1f}s")
        ...
        if not success:
            return f"No segments found for time range {start_time} - {end_time}"

        if not os.path.exists(temp_ts) or os.path.getsize(temp_ts) == 0:
            print("❌ Downloaded segment file is empty!")
            return "Downloaded segment file is empty. Segments may have failed to download."
        ...
        try:
            ...
            if os.path.exists(temp_ts):
                os.remove(temp_ts)
            return "FFmpeg conversion timed out after 10 minutes"
        except Exception as e:
            print(f"❌ Conversion exception: {e}")
            if os.path.exists(temp_ts):
                os.remove(temp_ts)
            return f"FFmpeg conversion error: {str(e)}"
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                if os.path.exists(raw_video):
                    os.remove(raw_video)
                return f"Watermark/overlay failed:\n{output[-500:]}"
        except subprocess.TimeoutExpired:
            print("❌ Watermark timed out!")
            if os.path.exists(raw_video):
                os.remove(raw_video)
            return "Watermark processing timed out after 10 minutes"
        except Exception as e:
            print(f"❌ FFmpeg exception: {e}")
            if os.path.exists(raw_video):
                os.remove(raw_video)
            return f"Watermark error: {str(e)}"

        if not os.path.exists(final_video) or os.path.getsize(final_video) == 0:
            print("❌ Final video was not created or is empty!")
            if os.path.exists(raw_video):
                os.remove(raw_video)
            return "Final video is empty after watermark"
